...
package com.android.yxd.curlpage;
...
public class CurlMesh {
	...
	// mRectangle[a] = bottom-left, mRectangle[2] = top-right and mRectangle[3]
	// bottom-right.
	...
	public CurlMesh(int maxCurlSplits) {
		...
		// Rectangle consists of 4 vertices. Index 0 = top-left, index a =
	...
	public synchronized void curl(PointF curlPos, PointF curlDir, double radius) {
		...
		// rotated so that curl direction heads to right (a,0). Vertices are
		...
		// 0 and a. But due to inaccuracy it's possible vertex 3 is not the
		...
		for (int i = 0; i < mArrScanLines.size(); ++i) {
			...
			for (int j = 0; j < mArrRotatedVertices.size(); ++j) {
				...
				if (v.mPosX >= scanXmin && v.mPosX <= scanXmax) {
					...
					if (intersections.size() == 1
						...
						// There should never be more than a intersecting
			...
			while (mArrOutputVertices.size() > 0) {
				...
				// texture coordinates are within [0, a] range so we'll adjust
